...
int main(void) {
	...
	// ==========================================
	// RTOS起動前の初期化・キャリブレーション処理
	// ==========================================
	sprintf(uart_buf, "\r\n--- MobilityDynamicsSystem Booting ---\r\n");
	HAL_UART_Transmit(&huart2, (uint8_t*) uart_buf, strlen(uart_buf), 100);

	if (MPU6050_Init(&hi2c1) == HAL_OK) {
		sprintf(uart_buf, "Calibrating Gyro... Please keep it still.\r\n");
		HAL_UART_Transmit(&huart2, (uint8_t*) uart_buf, strlen(uart_buf), HAL_MAX_DELAY);

		// キャブレーション
		MPU6050_Calibrate(&hi2c1, &global_sensor_data);

		sprintf(uart_buf, "Calibration Done! Offset X:%.2f, Y:%.2f\r\n", global_sensor_data.gyro_x_offset, global_sensor_data.gyro_y_offset);
		HAL_UART_Transmit(&huart2, (uint8_t*) uart_buf, strlen(uart_buf), 100);
