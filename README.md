...
<p align="center">
  ...
  <br>
  <i>自作のC#ダッシュボード。STM32から115200bpsで送信されるテレメトリデータを100Hzでリアルタイム描画し、解析用CSVの出力を行う。</i>
</p>
...
下段のPWMグラフにおいて定常的なズレが見られるが、これはハードウェアの個体差や組み付け時の微細な重力負荷の違いによるモデル誤差です。
...
<p align="center">
  ...
  <br>
  <i>図2: 予測アルゴリズムによるロバスト性検証。モーターの時定数 $\tau$ が変動（0.04〜0.15）する条件下でも発振せずに整定している。</i>
</p>

未来予測アルゴリズムと速度型制御を協調させることで、通常であれば発振を引き起こす高いゲイン設定においても、環境変動に強い驚異的なロバスト性を実現しています。
...
実機波形の分析から、ギアの物理的な遊び（バックラッシュ）が約1.5度存在することが同定されました。そこで、安全マージンを持たせた2.0度を不感帯のしきい値に設定しています。図3の波形頂点が平坦になっているのは、このハードウェアの限界に起因する激しいハンチング（発振）を防ぐため、あえて微小角での追従遅れを許容した安全設計（トレードオフ）の結果です。
