...
## 1. Background
本プロジェクトは、制御システム開発における**机上シミュレーションと実機挙動のギャップ検証**を目的として開発した1軸姿勢制御システムです。<br/>
単に実機を動かしてパラメータを調整する（現物合わせ）のではなく、Pythonによるプラントモデルの構築とシミュレーション、C#を用いた自作ツールによるデータドリブンなシステム同定を実施し、ハードウェア制約を含めた実用的な制御環境の構築を行っています。

## 2. Requirements
本開発において、以下の要件を満たす制御システムの構築を目指しました。
1. **外乱抑制:** ステップ応答時のオーバーシュートを最小限に抑え、速やかに目標角度へ整定すること。
2. **安定性の確保:** 実機特有の摩擦やバックラッシュによるハンチングを防ぎ、微小な定常偏差を収束させること。
3. **フェールセーフ:** 通信断絶などの異常時には、物理的な暴走を防ぐため即座に出力を遮断すること。
...
**■ RTOS タスク・キュー設計**<br/>
10msの厳格な制御周期を維持するため、I2Cセンサ読み取りとPID演算を最優先タスク（High）で実行。<br/>
時間のかかるUART送信処理は `osMessageQueue` を介して通常優先度タスク（Normal）へ逃がす、リアルタイム性を重視した非同期ロギングを実装しています。
...
システム全体をステートマシンで管理。正常稼働時（STATE_NORMAL）は誤差量に応じたゲインスケジューリングにより制御ゾーンを遷移します。<br/>
I2C通信異常（物理的な切断など）の検知時、または上位PCからの緊急停止指令受信時は即座に STATE_ERROR へ移行し、ハードウェアタイマーのPWM出力を完全停止（フェールストップ）させます。
...
### 4.2 アクチュエータ特性を考慮した「速度型制御」とゲインスケジューリング
**【課題】** <br/>
RCサーボ（SG90）内蔵の位置制御ICと、STM32からの位置型PID指令が干渉し、速応性を高めるとハンチングが発生してしまう課題がありました。

**【解決策】** <br/>
このハードウェア制約に対応するため、以下のアプローチを実装しました。

1. **速度型PIDアルゴリズムへの移行:**
   操作量の増分 $\Delta u(k)$ を算出し、出力をインクリメント更新する構成に変更しました。指令値を「速度」として扱うことでサーボ内部ICとの干渉や積分ワインドアップを回避し、高いゲインでも発振しない追従を実現しています。

2. **誤差に応じたゲインスケジューリング:**
   目標角度との偏差に基づき、4つのゾーンで動的に制御を切り替えます。
   * **大ズレ時:** バンバン制御（最大出力）で速応性を確保
   * **接近フェーズ:** 高い $K_p$ で目標値へ追従
   * **着地フェーズ:** 誤差の変化率に基づく $K_d$ で制動（ソフトランディング）
   * **不感帯内:** $K_i$ による定常偏差の吸収
...
図2の通り、モーターの時定数 $\tau$ が変動（0.04〜0.15）する条件下においても、ハンチングを引き起こすことなく整定しています。<br/>
速度型PIDに加え、現在の動作速度から先回りして制動（D動作）をかける仕組みを取り入れることで、ハードウェアの個体差や劣化による特性変化が起きても、ハンチングせずに安定して整定するロバスト性を確認しました。

### 4.5 追従性と安定性のトレードオフ設計
連続的なサイン波を用いたトラッキングテストを実施し、特定のステップ入力に対する過剰適合になっていないかを検証しました。
<p align="center">
  <!-- 画像挿入位置：sin波追従のテスト画像 -->
...
図3から、連続的な目標値変化に対しても安定して追従していることが分かります。
波形頂点が平坦になっている箇所は、ギアのバックラッシュ（実測約1.5度）に起因するハンチングを防止するため、不感帯のしきい値を意図的に2.0度に設定したことによるトレードオフの結果です。
